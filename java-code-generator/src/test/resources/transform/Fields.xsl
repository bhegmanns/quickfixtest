...
<xsl:call-template name="klassenbeschreibung"/>
...
{ 
  ...
  { 
     super(FIELD, data<xsl:if test="@type='UTCTIMESTAMP' or @type='UTCTIMEONLY'">, true</xsl:if>);
  } 
  ...
  { 
     super(FIELD, new <xsl:value-of select="$dataType"/>("" + data)); // higher precision
  } 
  </xsl:if>
}
...
<xsl:template name="klassenbeschreibung">

/**
* <xsl:value-of select="@name"/>-Tag.
*
* FIX-Tagnummer: <xsl:value-of select="@number"/>
* 
<xsl:choose><xsl:when test="@documentation">* Kurzbeschreibung:
*  <xsl:value-of select="@documentation"/></xsl:when></xsl:choose>
*
...
<xsl:choose>
  <xsl:when test="../@type='STRING'">public static final String <xsl:value-of select="@description"/> = "<xsl:value-of select="@enum"/>"; /*<xsl:value-of select="@documentation"/>*/
  </xsl:when>
  <xsl:when test="../@type='MULTIPLESTRINGVALUE'">public static final String <xsl:value-of select="@description"/> = "<xsl:value-of select="@enum"/>"; /*<xsl:value-of select="@documentation"/>*/
  </xsl:when>
  <xsl:when test="../@type='BOOLEAN'">public static final boolean <xsl:value-of select="@description"/> = <xsl:call-template name="y-or-n-to-bool" />;  /*<xsl:value-of select="@documentation"/>*/
  </xsl:when>
  <xsl:when test="../@type='INT'">public static final int <xsl:value-of select="@description"/> = <xsl:value-of select="@enum"/>; /*<xsl:value-of select="@documentation"/>*/
  </xsl:when>
  <xsl:when test="../@type='EXCHANGE'">public static final String <xsl:value-of select="@description"/> = "<xsl:value-of select="@enum"/>"; /*<xsl:value-of select="@documentation"/>*/
  </xsl:when>
  <xsl:when test="../@type='MONTHYEAR'">public static final String <xsl:value-of select="@description"/> = "<xsl:value-of select="@enum"/>"; /*<xsl:value-of select="@documentation"/>*/
  </xsl:when>
  <xsl:otherwise>public static final char <xsl:value-of select="@description"/> = '<xsl:value-of select="@enum"/>'; /*<xsl:value-of select="@documentation"/>*/
  </xsl:otherwise>
</xsl:choose>
